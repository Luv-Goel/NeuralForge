...
def _draw_cell(ax, edges, title):
    ...
    intermediates = sorted(list(set(
        idx for _, idx in edges
    )))
...
def plot_search_trajectory(
    ...
    fitness = [h.get("accuracy", h.get("fitness", h.get("val_accuracy", 0.0))) for h in history]
    ...
    ax1.plot(steps, best_so_far, color="#e17055", linewidth=2, linestyle="--", label="Best so far")
    ...
    if isinstance(fitness, list) and len(fitness) > 1:
        ...
        ax2.axvline(
            np.max(fitness), color="#e17055", linestyle="--",
            linewidth=2, label=f"Best: {np.max(fitness):.4f}",
        )
...
def _to_dot(genotype: Genotype) -> str:
    ...
    for node in [0, 1]:
        lines.append(f'  n{node} [label="c_k-{2-node}", fillcolor="#2d3436", fontcolor=white];')
